...
in
{
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf (! cfg.enable) {
      home.activation.enableKWin = lib.hm.dag.entryAfter [ "linkGeneration" ] ''
        if [ -e ~/.config/systemd/user/plasma-kwin_x11.service ]; then
          $DRY_RUN_CMD rm ~/.config/systemd/user/plasma-kwin_x11.service
        fi
      '';
    })
  ];
